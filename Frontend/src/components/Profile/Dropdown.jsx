...
const ProfileDropdown = ({ apiBaseUrl = "http://localhost:5000" }) => {
  ...
  const menuItems = [
    {
      category: "Account",
      items: [{ icon: User, label: "My Account", action: () => navigate("/myaccount") }],
    },
    {
      category: "Purchases",
      items: [{ icon: ShoppingBag, label: "All Purchases", action: () => navigate("/all-purchases") }],
    },
    {
      category: "Support",
      items: [
  ...
  // ✅ Fetch user info with token from localStorage
  useEffect(() => {
    const fetchUser = async () => {
      setLoading(true);
      try {
      ...
      {showDropdown && (
        <div className="absolute right-0 mt-2 w-80 bg-white dark:bg-zinc-900 text-black dark:text-white shadow-xl rounded-md py-4 px-4 z-50 transition-all duration-300">
          ...
          {menuItems.map((category, idx) => (
            <div key={idx} className="mb-3">
              {category.category && (
                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 uppercase font-medium">{category.category}</p>
              )}
              <div className="space-y-2 text-sm">
                {category.items.map((item, itemIdx) => (
                  <div
              ...
              {idx < menuItems.length - 1 && <hr className="my-3 border-gray-200 dark:border-gray-800" />}
            </div>
          ))}
