...
//       setLoading(true);
//       try {
//         const res = await fetch(`${apiBaseUrl}/api/auth/user/profile`, {
//           method: "GET",
//           credentials: "include",
//         });
...
//   // ✅ Fetch user info with token from cookies
//   useEffect(() => {
//     const fetchUser = async () => {
//       setLoading(true);
//       try {
...
//         ) : (
//           <div
//             className={`${getProfileColor()} w-8 h-8 rounded-full flex items-center justify-center text-white font-semibold`}
//           >
//             {getInitials(user?.name)}
//           </div>
//         )}
...
const ProfileDropdown = ({ apiBaseUrl = "http://localhost:5000" }) => {
  ...
  // Fetch user info using cookies only
  useEffect(() => {
    const fetchUser = async () => {
      setLoading(true);
      try {
        ...
        if (res.ok) {
          ...
          // Set user data based on your API response structure
          setUser(data); // If API returns user object directly
          // OR if your API wraps the user in a property:
          // setUser(data.user);
        } else {
          const errorData = await res.json();
          console.log("User fetch failed:", errorData.message);
        }
      } catch (err) {
        console.error("Error fetching user:", err);
      } finally {
